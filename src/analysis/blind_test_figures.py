...
# Training-set comparison: all-trained MutPred-PPI models on VarChAMP blind test (no 10-fold)
...
def plot_comparison(methods_data, save_name="roc_varchamp_blind_test.png"):
    ...
    for class_idx, c in enumerate([1, 2, 3]):
        ...
        for method_name, data in methods_data.items():
            ...
            # Shared NaN policy (see analysis.gcv_curves.class_roc_auc). This
            # used to call roc_curve with no mask at all. The blind test builds
            # tensors with require_complete=False, so an unscored row arrives
            # here as NaN rather than raising -- it should be empty now that
            # structure-less rows are dropped at dataset-build time, but a
            # silent NaN reaching roc_curve is not how we want to find out.
            fpr, tpr, roc_auc = class_roc_auc(y_scores, y_true)
            if fpr is None:
                continue
    ...
    for m in all_data:
        ...
        for c in [1,2,3]:
            if f"labels_c{c}" in all_data[m]:
                _, _, a = class_roc_auc(all_data[m][f"preds_c{c}"],
                                        all_data[m][f"labels_c{c}"])
                aucs[c] = a
