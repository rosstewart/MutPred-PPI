...
Usage:
    # NeuroDev only
    python map_neurodev.py --mode neurodev \
    ...
    # Fu et al. ASD only
    python map_neurodev.py --mode asd \
        --asd-dir asd \
        --asd-fasta asd/asd_uniprots.fasta \
        --biogrid-dir biogrid \
        --output-dir $MUTPRED_DATA_ROOT/asd

    # Both
    python map_neurodev.py --mode both \
        --neurodev-case autism/neurodev_case.fasta \
        --neurodev-control autism/neurodev_control.fasta \
        --neurodev-refseq-to-uniprot autism/neurodev_refseq_to_uniprot.tsv \
        --asd-dir asd \
        ...
        --asd-output-dir $MUTPRED_DATA_ROOT/asd
"""
...
def run_asd(args, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    print("=== Fu et al. de novo ASD mode ===")

    # Load variant TSV files
    df1 = pd.read_csv(os.path.join(args.asd_dir, "Fu_variants_SebatLab.tsv"), sep="\t")
    df2 = pd.read_csv(os.path.join(args.asd_dir, "Fu_variants_LIT.tsv"), sep="\t")
    ...
    if args.asd_gene_to_uniprot and os.path.exists(args.asd_gene_to_uniprot):
        with open(args.asd_gene_to_uniprot, "rb") as f:
        ...
        pkl_out = os.path.join(output_dir, "asd_gene_symbol_to_uniprot.pkl")
...
def main(args):
    ...
    if mode in ("asd", "both"):
        out = args.asd_output_dir or args.output_dir
        run_asd(args, out)
...
if __name__ == "__main__":
    ...
    p.add_argument("--mode", choices=["neurodev", "asd", "both"], default="both",
    ...
    # Fu et al. ASD inputs
    p.add_argument("--asd-dir", default=None,
                   help="Directory with Fu_variants_SebatLab.tsv and Fu_variants_LIT.tsv")
    p.add_argument("--asd-fasta", default=None,
                   help="UniProt FASTA for Fu et al. ASD proteins (optional for seq lookup)")
    p.add_argument("--asd-gene-to-uniprot", default=None,
                   help="Pre-built gene→UniProt pickle for Fu et al. ASD genes")
    # BioGRID
    ...
    p.add_argument("--asd-output-dir", default=None,
                   help="Output directory for Fu et al. ASD results")
    main(p.parse_args())
