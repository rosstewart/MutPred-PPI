...
Migrates `map_tulika_autism_variants.ipynb`, which lived outside version control.
This is the "tulika_autism" inference set: de novo missense variants from the Fu
et al. autism study, in two tables (SebatLab and LIT) that are simply
concatenated. It is a DIFFERENT dataset from `map_autism.py`, which handles the
older ASD/NDD case-control set; the two are not interchangeable.
...
Points worth stating because they look like bugs and are not:
  ...
  * Sequences come from BioGRID's `uniprot_dirbind_to_seq.pkl`, NOT from
    `tulika_autism_uniprots.fasta`. The notebook parsed that FASTA into a
...
Usage:
    python map_tulika_autism.py \\
        --variant-dir $MUTPRED_DATA_ROOT/tulika_autism \\
        --biogrid-dir $MUTPRED_DATA_ROOT/biogrid \\
        --af-pdb-dir /data/ross/alphafold_v4_human \\
        --af-pdb-pattern 'AF-{uniprot_id}-F1-model_v4.pdb.gz' \\
        --output-dir ./scratch_tulika_autism
"""
...
def build_gene_symbol_to_uniprot(df_hgnc, af_pdb_dir, af_pdb_pattern,
    ...
    `/data/ross/alphafold_v4_human_unzipped` (the path the notebook used) has
    been deleted; the gzipped mirror works via `--af-pdb-dir/--af-pdb-pattern`.
    """
...
def main() -> int:
    ...
    ap.add_argument("--af-pdb-dir", default="/data/ross/alphafold_v4_human_unzipped",
    ...
    ap.add_argument("--output-dir", default="./scratch_tulika_autism")
    args = ap.parse_args()
