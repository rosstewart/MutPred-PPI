...
def model_predict_subgraph(node_emb, edge_index_np, models, mut_local_idx, mutation_site_diff_np, device):
    ...
    """
    try:
        # from_numpy + .to() rather than torch.tensor(): torch.tensor() performs
        # the dtype cast on the CPU through the intra-op thread pool, and at 72
        # threads the OpenMP barrier costs ~20 ms on arrays this small -- 9x the
        # rest of the per-variant work combined. The GAT's scatter-aggregation
        # uses atomics and is already nondeterministic at ~2e-7 between identical
        # runs, so this stays inside the existing noise floor.
        x = torch.from_numpy(np.ascontiguousarray(node_emb, dtype=np.float32)).to(device)
        edge_index = torch.from_numpy(np.ascontiguousarray(edge_index_np, dtype=np.int64)).to(device)
        mut_diff_t = torch.from_numpy(np.ascontiguousarray(mutation_site_diff_np, dtype=np.float32)).to(device)
