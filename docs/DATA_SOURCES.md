...
| SAAMBE-3D (comparator method) | Iqbal et al. 2021, Nucleic Acids Research | Sequence- and structure-based ΔΔG predictor; checkout from http://compbio.clemson.edu/SAAMBE-3D/ (commit 182a2746). Ships its own SKEMPI-trained `*_v01.model` XGBoost boosters and requires `prody`; see REPRODUCING_ANALYSES.md. |
...
`notebooks/map_ppi_datasets_090826.py` reads eight files across two gitignored
