...
Structures are resolved by chain sequence from the canonical structure manifest
(`utils.structures.Structures`), not by filename: the previous route took a
sequence-matched `.mat` and then transformed its FILENAME into a structure name,
which reintroduced the coupling the sequence match had just removed. Mutation
positions are 1-based (from the canonical tables), so the API format
"A {WT}{pos1}{MT}" requires no +1 adjustment. The interactor's chain id comes
from the manifest -- it is not assumed to be chain A.
...
def run(args: argparse.Namespace) -> None:
    ...
    # Named, counted reasons rather than silent drops: a pair with no structure
    # and a pair whose structure will not convert are different coverage gaps.
    skipped: Counter = Counter()

    print(f"Dataset: {cfg.name}  rows: {len(rows)}  "
          f"structures indexed: {structures.n_indexed}", flush=True)
    ...
    for _, _train_idx, test_idx in fold_splits:
        for idx in test_idx:
            ...
            if key not in complex_to_pdb:
                # (path, interactor chain id); the API is uploaded a PDB, so a
                # gzipped mmCIF from the canonical tree is converted once here.
                complex_to_pdb[key] = structures.find_pdb(
                    interactor=row["interactor_sequence"],
                    partner=row["partner_sequence"])
                if complex_to_pdb[key][0] is None:
    ...
    for (interactor, partner), mutations in sorted(complex_to_mutations.items()):
        ...
        if pdb_path is None:
            print(f"  MISSING STRUCTURE: {interactor}/{partner}", flush=True)
            skipped["rows_without_structure"] += len(mutations)
