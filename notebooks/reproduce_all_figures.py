...
#
# **Only `datasets/mapped090826/` data is ever read.** Every dataset name is
...
#
# **Not reproducible here, by design (see `docs/ZENODO_MANIFEST.md`):**
# Fig 4/S2 and Table 1 need unpublished VarChAMP data, present only in
# `datasets/mapped090826/varchamp_all_mapped090826_rows.csv.gz`; those steps
...
from paths import DATA_ROOT, DATASETS_DIR, REPO_ROOT, WEIGHTS_DIR, describe  # noqa: E402
...
_MAPPED = DATASETS_DIR / "mapped090826"
...
run([PY, "repro_test/build_af3_index.py"],
   produces=[_MAPPED / "af3_index.csv.gz"], name="build_af3_index.py")
run([PY, "repro_test/build_canonical_tables.py"],
   produces=[_MAPPED / "sahni_fragoza_mapped090826_rows.csv.gz"],
   name="build_canonical_tables.py")
...
# docs/ZENODO_MANIFEST.md). The unpublished-data concern moved WITH the data:
...
if not _VARCHAMP_AVAILABLE:
    print("[skip] Step 6: datasets/mapped090826/varchamp_all_mapped090826_rows.csv.gz "
          "is absent -- Fig 4 and S2 are not independently reproducible without "
          "unpublished VarChAMP data (docs/ZENODO_MANIFEST.md).")
else:
