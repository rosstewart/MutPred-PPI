...
# Fig 4, S3, S6 and Table 1 need unpublished VarChAMP data, present only in
# `datasets/training_eval/varchamp_all_mapped090826_rows.csv.gz` and the pooled
# `sahni_fragoza_varchamp_all_...` tables; those steps detect its absence and
# skip with a clear message rather than failing. S2 is the Sahni-only GCV and
# reproduces without it. Figs
...
# Folds per seed. The splits are 10-fold; QUICK keeps one, which is another 10x
# on top of N_GCV. Every trained method takes --n-folds through the shared
# runner (utils/gcv_common.run_gcv), so this applies uniformly. A published run
# must use all 10.
N_FOLDS = 1 if QUICK else None
...
# QUICK-mode cap for the SKEMPI-pretrained comparators (SAAMBE-3D, MutPPI,
# MutPPI+). They score every row once instead of cross-validating, so N_GCV does
# not shrink them: SAAMBE-3D alone is ~3 s/row because upstream is invoked as one
# subprocess per row. Unscored rows stay NaN, which every consumer already masks.
N_PRETRAINED_ROWS = 200
...
from paths import (CONTACT_GRAPH_STORE, DATA_ROOT, DATASETS_DIR,  # noqa: E402
                   MAPPING_DIR, REPO_ROOT, TRAINING_EVAL_DIR, WEIGHTS_DIR,
                   describe)
...
# Step 4 runs each comparison method from its own upstream checkout under
# external_methods/. Report what is missing here rather than 40 minutes into
# cross-validation, and name the document that says how to get it.
_METHOD_DIRS = {"esignet": "eSIG-Net", "mint": "MINT", "PPLM": "PPLM",
                "MutPPI": "MutPPI / MutPPI+", "saambe3d": "SAAMBE-3D"}
_METHODS_ROOT = Path(os.environ.get("MUTPRED_PPI_METHODS_DIR",
                                    REPO / "external_methods"))
_missing_methods = [f"{d} ({label})" for d, label in _METHOD_DIRS.items()
                    if not (_METHODS_ROOT / d).exists()]
if _missing_methods:
    print(f"[note] {_METHODS_ROOT} is missing {len(_missing_methods)} comparison "
          f"method(s): {', '.join(_missing_methods)}.\n"
          f"       Step 4 skips whichever it cannot find. To run them, see "
          f"'Comparison methods: clone each upstream repository' in "
          f"docs/REPRODUCING_ANALYSES.md.\n"
          f"       SWING needs no checkout (src/evaluation/swing_common.py) and "
          f"MutPred2 runs off-machine.")
else:
    print(f"comparison methods: all {len(_METHOD_DIRS)} present in {_METHODS_ROOT}")
...
#
# `requires` the mapping CSVs, which only notebooks/map_ppi_datasets.py
# produces and which the Zenodo deposit does not carry. A reader working from
# the deposit alone has nothing for this step to annotate: the deposited
# datasets/training_eval/ row tables were built with the flag already applied,
# so the pairs it would mark are already gone. Without this guard the step
# exits 1 and takes the whole notebook down at Step 2.
run([PY, "src/data_processing/annotate_af3_coverage.py"],
   requires=[MAPPING_DIR / "datasets"],
...
# Keep only the datasets whose row table is actually present. The two
# VarChAMP-derived tables are not redistributable, so a reader working from the
# Zenodo deposit has three of the five; without this filter every step below
# that loops over _GCV_DATASETS dies on the first missing one.
_GCV_DATASETS, _GCV_ABSENT = [], []
for _b in _GCV_BASES:
    _ds = dataset_name(_b)
    (_GCV_DATASETS if (_MAPPED / f"{_ds}_rows.csv.gz").exists()
     else _GCV_ABSENT).append(_ds)
if _GCV_ABSENT:
    print(f"[skip] no row table for: {', '.join(_GCV_ABSENT)} -- these need the "
          f"unpublished VarChAMP measurements (see docs/DATA.md)")
...
#
# Each comparison method runs from its own upstream checkout under
# `external_methods/`; Step 0 reports which are present. The `git clone` lines
# and the commits used for the published results are in
# `docs/REPRODUCING_ANALYSES.md`, under "Comparison methods: clone each upstream
# repository". SWING is implemented in-repo and needs no checkout.
#
...
for ds in _GCV_DATASETS:
    ...
    for pred in ("seq_diff", "site_diff"):
        run([PY, "src/evaluation/mint_cv.py",
            "--dataset", ds, "--predictor", pred, "--n-gcv", str(N_GCV)] + _FOLD_ARGS,
    ...
    # SWING refits Doc2Vec per fold and is the slowest method here, so it needs
    # --n-gcv like every other one. It used to hardcode 30 seeds, which meant
    # QUICK mode ran 1 seed of everything else and 30 of this.
    run([PY, "src/evaluation/swing_gcv.py", "--dataset", ds, "--n-gcv", str(N_GCV)]
       + _FOLD_ARGS,
       produces=[_GCV_DIR / f"SWING_{ds}_no_test_pretrain_detailed_results.pkl"],
       name=f"swing_gcv.py [{ds}, blind-test]")
    run([PY, "src/evaluation/swing_gcv.py", "--dataset", ds, "--n-gcv", str(N_GCV),
        "--test-pretrain"] + _FOLD_ARGS,
       produces=[_GCV_DIR / f"SWING_{ds}_test_pretrain_detailed_results.pkl"],
       name=f"swing_gcv.py [{ds}, test-pretrain]")
    # SAAMBE-3D, MutPPI and MutPPI+ are pretrained on SKEMPI: they score every
    # row once rather than cross-validating, so --n-gcv does not apply and the
    # cost does not fall with N_GCV. In QUICK mode they are capped instead;
    # unscored rows stay NaN, which is what a row with no structure already
    # gets, so the arrays keep their length and the figures still draw.
    _PRETRAINED_CAP = ["--max-rows", str(N_PRETRAINED_ROWS)] if QUICK else []
    run([PY, "src/evaluation/saambe3d_cv.py", "--dataset", ds,
        "--outdir", str(_GCV_DIR)] + _PRETRAINED_CAP,
    ...
    for variant, stem in ((0, "MutPPI"), (1, "MutPPIPlus")):
        run([PY, "src/evaluation/mutppi_cv.py",
            "--dataset", ds, "--model", str(variant), "--device", DEVICE,
            "--outdir", str(_GCV_DIR)] + _PRETRAINED_CAP,
...
if not _VARCHAMP_AVAILABLE:
    print(f"[skip] Step 6: {_VARCHAMP_ROWS} is absent -- Fig 4 and S3 are not "
...
#
# The published tier scores every variant repository with the all-data model,
# trained on every labelled pair with no fold split, and writes to
# `results/variant_dbs_all_data/`.
# `run_variant_db_inference.py::assert_all_data_model` hashes the checkpoint and
# refuses to run with anything else, so there is only ever one published
# variant-DB results tree.
#
# Retraining that model needs the unpublished VarChAMP measurements. **Scoring
# with it does not**: `weights/MutPred-PPI.pt` is in the Zenodo weights bundle.
# So there are three routes through this step, and which one you get depends on
# what is on disk:
#
# | | retrain the all-data model | score with it | writes to |
# |---|---|---|---|
# | VarChAMP present | yes | yes | `variant_dbs_all_data/` |
# | deposit only | no | yes, deposited checkpoint | `variant_dbs_all_data/` |
# | no all-data checkpoint | no | Sahni+Fragoza instead | `variant_dbs_sahni_fragoza/` |
#
# The third is a DEMONSTRATION: it runs variant-repository inference end to end
# so you can see what it produces, but its scores are not the paper's. Every
# figure it draws is stamped, so the tiers cannot be confused.

# %%
_ALL_DATA_CKPT = WEIGHTS_DIR / "MutPred-PPI.pt"   # the fixed name every consumer resolves
_SF_CKPT = WEIGHTS_DIR / "sahni_fragoza" / "MutPred-PPI.pt"

# Retraining and scoring are separate capabilities. Gating the output tree on
# VarChAMP conflated them and sent deposit-only readers to the demonstration
# tier even with the published checkpoint sitting in weights/.
_CAN_RETRAIN_ALL_DATA = _VARCHAMP_AVAILABLE
_PUBLISHED_TIER = _CAN_RETRAIN_ALL_DATA or _ALL_DATA_CKPT.exists()
_DEMO_TIER = not _PUBLISHED_TIER

_VDB_DIR = _RESULTS_DIR / ("variant_dbs_all_data" if _PUBLISHED_TIER
                           else "variant_dbs_sahni_fragoza")
_VDB_STAB_DIR = _RESULTS_DIR / "variant_dbs_stability"
_TIER_ARGS = [] if _PUBLISHED_TIER else ["--model-tier", "sahni_fragoza"]

if _DEMO_TIER:
    if not _SF_CKPT.exists():
        print(f"[skip] Steps 7-9: neither the all-data checkpoint "
              f"({_ALL_DATA_CKPT}) nor the Sahni+Fragoza demonstration model "
              f"({_SF_CKPT}) is present. Both are in the Zenodo weights bundle; "
              f"see docs/DATA.md.")
    else:
        print("=" * 72)
        print("DEMONSTRATION TIER: no all-data checkpoint, using the")
        print("Sahni+Fragoza model. Results go to")
        print("results/variant_dbs_sahni_fragoza/ and are NOT the published")
        print("numbers. See the 'What you can reproduce' table in README.md.")
        print("=" * 72)
elif not _CAN_RETRAIN_ALL_DATA:
    print("=" * 72)
    print("PUBLISHED TIER, from the deposited checkpoint. VarChAMP data is")
    print("absent, so the all-data model cannot be retrained here, but scoring")
    print("with it reproduces the published variant-repository numbers.")
    print("=" * 72)

if _CAN_RETRAIN_ALL_DATA:
...
if _PUBLISHED_TIER or _SF_CKPT.exists():
    for db in ("clinvar", "gnomad", "cosmic", "hgmd", "neurodev", "asd"):
        ...
        try:
            # Gate on the completion sentinel, not the TSV. The script appends
            # as it goes, so a crashed run leaves a short (often header-only)
            # file behind; `produces=` would then call it cached forever and
            # every downstream step would read an empty predictions table.
            # `{out}.complete` is written only once the row iterator is
            # exhausted -- that is what it exists for (see `_report`), and
            # src/run_benchmarks.py already gates on it.
            run(cmd, produces=[_VDB_DIR / f"{db}_mutpred_ppi_predictions.tsv.complete"],
               name=f"run_variant_db_inference.py [{db}]")
            stab_cmd = [PY, "src/variant_db_inference/run_stability_inference.py",
                        "--dataset", db, "--device", DEVICE,
                        "--out-dir", str(_VDB_STAB_DIR)]
            if QUICK:
                # The same subsample the interaction predictions used. S10 joins
                # the two, so scoring different row sets would have made its two
                # axes disagree on which variants they cover.
                stab_cmd += ["--rows", str(subsample_rows(db, DB_SUBSAMPLE))]
            run(stab_cmd,
...
if _VDB_READY:
    ...
    # No --panels: the two-panel rebuild does not take one, and passing it made
    # this step exit 2 every time, so S10 was never drawn by the notebook.
    # --n-bootstrap so QUICK shrinks it like every other bootstrap here.
    run([PY, "src/analysis/stability_interaction_scatter.py",
        "--data-dir", str(_VDB_DIR), "--n-bootstrap", str(N_BOOTSTRAP)] + _DEMO_ARGS,
